...
DROP VIEW IF EXISTS kurs_overview;
...
VALUES
    ...
    (511127, 3.0, 5, STR_TO_DATE('22.12.21 23:59', '%d.%m.%y %H:%i'), 0, NULL, FALSE),
...
WHERE fk_matnr = 511127 AND fk_kurs = 7;
...
WHERE a.id NOT IN (
    ...
    FROM leistung
    WHERE fk_matnr = p_mat_nr)
  AND fk_kurs = p_kurs_id;
END //
DELIMITER ;

-- Übersicht Latedays Leistungen
CALL show_due_dates_with_late_days(511127, 7);

-- Latedays übrig, nicht negativ
SELECT calc_latedays_left_total(SUM(verbrauchteld), latedays_verfuegbar) AS ld_calc, fk_matnr
    FROM latedays_merged_overvies lo
             JOIN abgabe_in_kurs ak ON ak.id = lo.abgabe_id
    WHERE fk_matnr = 511127
      AND kurs_id = 7;
